...
<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%
	String seMemMail = (String) session.getAttribute("memMail");
							...
							<div class="single-header style-2">
								<div class="row">
									<div class="col-xl-10 col-lg-12 m-auto">
										<h6 class="mb-10">
											<a href="<%=request.getContextPath()%>/listQna.do">문의</a>
										</h6>
										<h6><%=qv.getMemMail()%></h6>
										...
										<div class="entry-bottom mt-50 mb-30">
											<div class="tags w-50 w-sm-100">
												<a href="listQna.do" rel="tag"
													class="hover-up btn btn-sm btn-rounded mr-10">목록</a>
												<%
													if (seMemMail != null && seMemMail.equals(qnaMail)) {
												%>
												<a href="updateQna.do?qnaNo=<%=qv.getQnaNo()%>"
													class="hover-up btn btn-sm btn-rounded mr-10">수정</a> <a
													href="deleteQna.do?qnaNo=<%=qv.getQnaNo()%>"
											...
											<div class="row">
												<div class="col-lg-9 col-md-12  m-auto">
													...
													<form onsubmit="return validCheck()"
														class="form-contact comment_form mb-50"
														action="<%=request.getContextPath()%>/qnaDetail.do"
														method="post">
															...
															<div class="col-12">
																<div class="form-group">
																	<textarea class="form-control w-100"
																		name="comentContent" cols="30" rows="9"
																		placeholder="답변을 남겨주세요." id="comentCon"></textarea>
																</div>
															</div>
														</div>
														<input type="submit"
															class="hover-up btn btn-rounded mr-10" value="등록">
													</form>
																		...
																		<div class="d-flex justify-content-between mb-10">
																			<div class="d-flex align-items-center">
																				...
																				<div id="d1">
																					...
																					%>
																					<a
																						href="<%=request.getContextPath()%>/updateComent.do?comentNo=<%=comentList.get(i).getComentNo()%>">수정</a>
																					<a
																						href="<%=request.getContextPath()%>/deleteComent.do?comentNo=<%=comentList.get(i).getComentNo()%>&qnaNo=<%=qv.getQnaNo()%>">삭제</a>
																					<%
																						} else {
																					%>
																					<a
																						href="<%=request.getContextPath()%>/updateComent.do?comentNo=<%=comentList.get(i).getComentNo()%>"
																						style="display: none">수정</a> <a
																						href="<%=request.getContextPath()%>/deleteComent.do?comentNo=<%=comentList.get(i).getComentNo()%>&qnaNo=<%=qv.getQnaNo()%>"
																						style="display: none">삭제</a>
																					<%
																						}
																					%>
