...
@WebServlet("/MylectureList.do") // value = <이거 빠져도 오류가 안난다.
// 이걸 하고나서 web.xml에 또 등록을 하면 충돌이 난다. 그러므로 web.xml에 하거나 이걸 하거나 둘중 하나 택해야.
public class PayListMemberController extends HttpServlet { // tomcat을 참조하면 이렇게 나온다.
...
//		List<MemberVO> memList = memService.getAllMemberList();
		...
		RequestDispatcher rd = 
				req.getRequestDispatcher("/WEB-INF/jsp/member/mypage/mypagePayList.jsp");
