...
private static ILectureDAO lecDAO;
	...
	public List<LectureVO> getAllLectureList2(SqlMapClient smc) {
		...
		try {
			...
			lecList = smc.queryForList("lecture.getLectureAll2");
	...
	public LectureVO getLecture2(SqlMapClient smc, String lecId) {
		...
		try {
			lv = (LectureVO)smc.queryForObject("lecture.getLecture2", lecId);
		} catch (SQLException e) {
			e.printStackTrace();
			throw new RuntimeException("강의정보조회 중 예외 발생!", e);
		}
	...
	public List<LectureVO> getCateLectureList(SqlMapClient smc) {
		...
		try {
			...
			lecList = smc.queryForList("lecture.getCateLectureLsit");
