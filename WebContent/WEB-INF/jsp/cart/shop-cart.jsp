...
%>
								...
								<tr id="checkTr">
									...
									<input class="form-check-input" type="checkbox" name="checkbox" id="<%=lec.getLecId()%>" value="<%=lec.getLecPrice()%>" onclick="test(this)">						
									<label class="form-check-label" for="<%=lec.getLecId()%>">
									...
									<td class="product-des product-name">
										<h6 class="mb-5">
											<a class="product-name mb-10 text-heading"
												href="<%=request.getContextPath()%>/lecture/detail.do?lecId=<%=lec.getLecId() %>" style="text-align: center"><%=lec.getLecTitle()%> </a>
										</h6>
...
<!-- 										</div> -->
									...
									<td class="product-des product-name">
										<h6 class="mb-5">
											<a class="product-name mb-10 text-heading"
												href="<%=request.getContextPath()%>/lecture/detail.do?lecId=<%=lec.getLecId() %>" style="text-align: center"></a>
										</h6>
									</td>
									...
									<a href="<%=request.getContextPath()%>/cart/cartDelete.do?i=<%=i %>" class="text-body"><i class="fi-rs-trash"></i></a></td>
								</tr>
<%
		}
%>
...
function test(obj) {
	...
	$("input[type=checkbox]").each(function(index, item){
		if($(this).is(":checked")){ //대상이 체크 되어 있을 때
			...
			if(checklist == ''){
				...
				checklist = checklist + ',' + $(this).attr('id');
			}	
		}
	});
    ...
    for (var i=0; i<obj_length; i++) {
        if (document.getElementsByName("checkbox")[i].checked == true) {
            ...
            sum = sum + intSum;
            check = $(obj).prop('checked');
        }
    }
/*     var NaN = isNaN(intSum); */
/*       alert(sum); */
	  document.getElementById('sum_t_sum').innerText = "총 결제금액 : " + sum + "원";
}
</script>
<!-- 				결제 시작-->
				<div class="col-lg-4" style="padding-right: 80px;">
					<div id="countBox">
	                	...
	                	<div class="row">
	                        <h4 class="mb-30" id="sum_t_sum">총 결제금액 : 0원</h4>
	                        <form method="post">
...
function input(){
	...
	console.log(memName,memTel);
}

IMP.init("imp95268200"); // 예: imp00000000
   function requestPay() {
    	  $.ajax({
			  url : '<%=request.getContextPath()%>/pay/insertOrder.do',
			  type : 'post',
			  data : {
						  orderPrice: sum,
						  memMail: "<%=memMail%>"
			  ...
			  success : function(rst,status,xhr){
				  ...
				  IMP.request_pay({ // param
			          ...
			          if (rsp.success) {
			        	  ...
			        	  suc = "T";
						  $.ajax({
							  url : '<%=request.getContextPath()%>/pay/insertPay.do',
							  type : 'post', 		//method
							  data : {
										  payId	: oi,
										  paySuc : suc,
...
//					 	  error : function(1.xhr속성 / 2.데이터상태문자열 / 3. error코드 ){},
				    ...
				    if(result=='0') {
					   	console.log("1");
					      $.ajax({
					    	  url : '<%=request.getContextPath()%>/cart/insertCart.do',
// 					    	  url : '/DING/cart/insertCart.do',
					    	  type : 'post',
					    	  data : {
					    		  payId	: oi,
					    		  checklist: checklist
					    	  ...
					    	  success : function(rst,status,xhr){
					   			console.log("2");
// 					    		  $.ajax({
<%-- 					    			   	  url : '<%=request.getContextPath()%>/pay/Receipt.do',  --%>
// 					    			   		  type : 'post',
// 					    			   		  data : {
// 					    			   			  payId	: oi
// 					    			   		  },
// 					    			   		  success : function(rst,status,xhr){
// 					    			   		  console.log("Receipt로 통신!");
// 					    			   	  },
// 					    			   	  error : function(xhr,status,error){
// 					    			   		  console.log(xhr.status);
// 					    			   		  console.log(xhr.statusText);
...
// 					    			   	  dataType : 'json'
// 					    			   	  });
								console.log("InsertCartController로 통신!");
								location.href = '<%=request.getContextPath()%>/pay/Receipt.do?payId='+ oi;
							  },
							  error : function(xhr,status,error){
								  console.log(xhr.status);
								  console.log(xhr.statusText);
