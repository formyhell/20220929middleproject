...
<%--                                         <h6 class="mb-10"><a href="<%=request.getContextPath() %>/lecture/detail.do?lecId=<%=lv.getLecId()%>"><%=lv.getLecTitle() %></a></h6> --%>
                                        <form onsubmit="return validCheck()" action="<%=request.getContextPath() %>/updateMoreQna.do" method="post" onsubmit="return false">
                                        <input type="hidden" name="qnaNo" value="<%=qv.getQnaNo() %>">
                                        <input type="hidden" name="lecId" value="<%=qv.getLectureId() %>">
